...
export default function Queue({ items, onOpen }: { items: ContentItem[]; onOpen?: (id: string) => void }) {
  ...
  return (
    <div className="rounded-xl bg-surface-container-lowest border border-outline-variant shadow-card overflow-hidden">
      <div className="px-5 py-4 text-sm font-medium text-on-surface border-b border-outline-variant/30">
        Needs Action
      </div>
      <ul className="divide-y divide-outline-variant/20">
          ...
          <li key={it.id}>
            ...
            >
              <div className="min-w-0">
                <div className="text-[15px] font-medium text-on-surface truncate">{it.title}</div>
                <div className="text-[13px] text-on-surface-variant mt-0.5">
                  {it.publish_date ?? 'Not scheduled'}
                </div>
              </div>
        ...
        {pending.length === 0 && (
          <li className="px-5 py-8 text-center text-xs text-outline">No pending items</li>
        )}
      </ul>
    </div>
  )
